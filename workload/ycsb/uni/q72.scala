...
object SQLDataSourceExample {
  ...
  private def q1(spark: SparkSession): Unit = {
    ...
    val res = df
      .filter($"weighted_score"===11)
      .filter($"linear_score"===16)
      .filter($"weighted_score"===29)
      .filter($"weighted_score"===34)
      .filter($"weighted_score"===45)
      .filter($"linear_score"===47)
      .filter($"weighted_score"===76)
      .count()
    // scalastyle:off println
    System.out.println(res)
    val time2 = System.currentTimeMillis()
