...
object SQLDataSourceExample {
  ...
  private def q1(spark: SparkSession): Unit = {
    ...
    val res = df
      ...
      .filter($"weighted_score"===5)
      .filter($"linear_score"===25)
      .filter($"weighted_score"===28)
      .filter($"weighted_score"===35)
      .filter($"weighted_score"===39)
      .filter($"linear_score"===41)
      .filter($"weighted_score"===75)
      .count()
    // scalastyle:off println
    System.out.println(res)
    val time2 = System.currentTimeMillis()
