...
object SQLDataSourceExample {
  ...
  private def q1(spark: SparkSession): Unit = {
    ...
    val res = df
      .filter($"linear_score"===1)
      .filter($"weighted_score"===7)
      .filter($"weighted_score"===30)
      .filter($"linear_score"===45)
      .filter($"weighted_score"===58)
      .filter($"linear_score"===84)
      .filter($"weighted_score"===91)
      .filter($"linear_score"===97)
      .count()
    // scalastyle:off println
    System.out.println(res)
    val time2 = System.currentTimeMillis()
